...
public class Test3 extends JFrame{
	...
	class NorthPanel extends JPanel{ //상단 설명패널 클래스 설정 
		public NorthPanel() {
			...
			setBackground(new Color(104,104,104)); //상단 배경 
			...
			info.setFont(new Font("나눔명조",Font.PLAIN,20));
			info.setBackground(Color.blue); //배경색 
	...
	/*class My extends MouseAdapter{ //마우스 
		public void mousePressed(MouseEvent e)
		{
			if(e.getClickCount()==2) {
				flag=0;
				label.setFont(new Font("나눔고딕",0,40));
				label.setText("");
	...
	class ResultPanel extends JPanel{
		public ResultPanel() {
			...
			setBackground(Color.gray); //하단 배경색 
			//setForeground(Color.white); //안먹힘 
			...
			for(int i=0; i<2; i++)
			{
				if(i==0)
				{
					btt[0].setFont(new Font("Apple SD 산돌고딕 Neo",0,22));//하 =.c폰트 크기 
			...
			//btt[0].addActionListener(new MyListener());
	...
	class CenterPanel extends JPanel{ //센터패널(버튼출력부분) 
		public CenterPanel() {
			...
			setBackground(Color.gray); //중앙부 뒷배경 
			...
			for(int i=0; i<16; i++) 
			{
				if(i%4!=3 && i<11) //숫자 
				{
					...
					//bt[i].setForeground(Color.white);글씨가 뭍
					add(bt[i]);
					...
					bt[12].addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							...
							else if(0<n && n<=10)
							{
								...
								label.setFont(new Font("Apple SD 산돌고딕 Neo",0,35));
								label.setText(label.getText().substring(0,n));
								info.setText("수식을 지우는 중입니다.");
							}
				...
				else if(i==14) //.일 때 
				{
					...
					bt[i].setBackground(Color.orange); //적용 안됨 흑 
					//bt[i].setForeground(Color.white);
					add(bt[i]);
	...
	public class CalcListener implements ActionListener{ //계산의 결과를 확인해서 info에 해당 내용 출력 
		public void actionPerformed(ActionEvent e) {
			...
			if(result<0) {
				info.setFont(new Font("나눔고딕",0,14));
				info.setForeground(new Color(85,14,14));
			...
			{
				info.setFont(new Font("나눔고딕",0,13));
				info.setForeground(new Color(85,14,14));
    ...
    public double Calculator(String ss) { //계산! 누적 입력 된 스트링을 파라미터로 받아와서 계산 return 값 double 타입으로 존재 
    	int i;  
    	double ans; 
    	check = 0; 
    	ArrayList<Double> v = new ArrayList<Double>(); //숫자, 점을 입력할 arraylist 만들기 
    	ArrayList<String> op = new ArrayList<String>(); //연산자 저장해 놓을 arraylist 만들기 
    	...
    	for(i=0; i<ss.length();i++) {
    		...
    		else if(s.equals(".")) str+=s; //점이어도 str에 누적 붙이기(연산자나오기 전까지)
    		else { //연산자 나오면 
